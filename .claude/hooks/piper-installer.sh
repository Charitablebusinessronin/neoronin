...
# Linux/WSL: Try pipx, fallback to binaries
else
  ...
  if command -v pipx &> /dev/null; then
    ...
    if pipx install piper-tts; then
      INSTALL_SUCCESS=true
    fi
  fi

  if [[ "$INSTALL_SUCCESS" == false ]]; then
    echo "⚠️  pipx installation failed or pipx not found. Falling back to precompiled binaries..."
    echo ""
    ...
    if curl -L "$PIPER_URL" | tar -xz; then
      ...
      # Move contents to ~/.local/lib/piper
      cp -r piper/* "$LIB_DIR/"
      
      # Create wrapper script in ~/.local/bin
      cat > "$BIN_DIR/piper" << EOF
#!/bin/bash
LD_LIBRARY_PATH="\$HOME/.local/lib/piper:\$LD_LIBRARY_PATH" "\$HOME/.local/lib/piper/piper" "\$@"
EOF
      chmod +x "$BIN_DIR/piper"
      
      echo "✅ Installed Piper to: $LIB_DIR"
      echo "✅ Created wrapper at: $BIN_DIR/piper"
      INSTALL_SUCCESS=true
    else
      echo "❌ Failed to download Piper binaries"
      cd -
      rm -rf "$TEMP_DIR"
      # Don't exit yet, try one more thing or report failure
    fi
    cd -
    rm -rf "$TEMP_DIR"
  fi
  ...
  if [[ "$INSTALL_SUCCESS" == false ]]; then
      echo "❌ Unable to install Piper TTS automatically."
      echo ""
